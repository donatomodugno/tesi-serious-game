...
exports.clear = () => {
    db.serialize(() => {
        ...
        db.run(`DROP TABLE IF EXISTS results`, (err) => { if(err) console.error(err.message) })
    })
}
...
exports.addExercise = (exer) => {
    return new Promise((resolve, reject) => {
        const sql = "INSERT INTO exercises(name,turns,bpmn) VALUES(?,?,?)"
        db.run(sql, [exer.name, exer.turns, exer.bpmn], function (err) {
            if(err) {
                reject(err)
                return
            }
...
exports.editExercise = (exer) => {
    return new Promise((resolve, reject) => {
        const sql = "UPDATE exercises SET name=?,turns=?,bpmn=? WHERE id=?"
        db.run(sql, [exer.name, exer.turns, exer.bpmn, exer.id], (err) => {
            if(err) {
                reject(err)
                return
            }
...
exports.addCard = (card) => {
    return new Promise((resolve, reject) => {
        const sql = "INSERT INTO cards(ex_id,name,type,cost,score,bonus,draws,buys,turns,bpmn) VALUES(?,?,?,?,?,?,?,?,?,?)"
        db.run(sql, [card.ex_id, card.name, card.type, card.cost, card.score, card.bonus, card.draws, card.buys, card.turns, card.bpmn], function (err) {
            if(err) {
                reject(err)
                return
            }
...
exports.editCard = (card) => {
    return new Promise((resolve, reject) => {
        const sql = "UPDATE cards SET name=?,type=?,cost=?,score=?,bonus=?,draws=?,buys=?,turns=?,bpmn=? WHERE id=?"
        db.run(sql, [card.name, card.type, card.cost, card.score, card.bonus, card.draws, card.buys, card.turns, card.bpmn, card.id], (err) => {
            if(err) {
                reject(err)
                return
            }
...
exports.listExerciseResults = (ex_id) => {
    return new Promise((resolve, reject) => {
        const sql = "SELECT * FROM results WHERE ex_id=?"
        db.all(sql, [ex_id], (err, rows) => {
            if(err) {
                reject(err)
                return
...
exports.deleteExerciseResults = (ex_id) => {
    return new Promise((resolve, reject) => {
        const sql = "DELETE FROM results WHERE ex_id=?"
        db.run(sql, [ex_id], (err) => {
            if(err) {
                reject(err)
                return
